...
for i, service in enumerate(services):
    ...
    sns.lineplot(data=knowledge_data, x='start_time_stamp', y='min_cpu_quota_percentage_across_pods', label='adaptive', color='r', ax=ax_list[i*2])

    # Plot the 'max cpu' for knowledge_baseline.csv
    sns.lineplot(data=knowledge_baseline_data, x='start_time_stamp', y='min_cpu_quota_percentage_across_pods', label='baseline', color='b', ax=ax_list[i*2])
...
plt.savefig("cpu_saturation.png")
