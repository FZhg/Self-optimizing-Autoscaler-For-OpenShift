...
for i, service in enumerate(services):
    ...
    # Plot the 'latency_ms' for knowledge.csv
    sns.lineplot(data=knowledge_data, x='start_time_stamp', y='latency_ms', label='adaptive', color='r', ax=ax_list[i*2])

    # Plot the 'latency_ms' for knowledge_baseline.csv
    sns.lineplot(data=knowledge_baseline_data, x='start_time_stamp', y='latency_ms', label='baseline', color='b', ax=ax_list[i*2])
